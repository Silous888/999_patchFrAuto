...
def trouver_bin_zero_escape():
    ...
    for chemin in dossiers_jeux:
        chemin_complet = (
            chemin + "\\steamapps\\common\\Zero Escape The Nonary Games\\ze1_data.bin"
        )
        if os.path.exists(chemin_complet):
            return chemin_complet
    utils.logging.error("ze1_data.bin non trouvé")
    return -1
...
def copier_bin_dans_ZE999Tool(chemin_bin):
    ...
    """
    dossier_tool = "ZE999Tool.v0.9.8.1\\bin_org"
    ...
    copier_fichier_progression(
        chemin_bin, dossier_tool_complet + "\\ze1_data.bin", afficher_progression
    )
